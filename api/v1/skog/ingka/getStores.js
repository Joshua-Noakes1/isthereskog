...
async function getStores(ISOCode) {
    // log lookup
    console.log(lcl.blue("[Store Lookup - Info]"), "Looking up stores for country code:", lcl.yellow(ISOCode));

    // get stores for country code, if no country code is passed use GB
    try {
        const stores = await ingka.stores.findByCountryCode(ISOCode);
        ...
        // return stores
        console.log(lcl.green("[Store Lookup - Success]"), "Found stores for country code:", lcl.yellow(ISOCode));
        return {
            success: true,
            stores
        }
